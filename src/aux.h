...
static int CPXPUBLIC callback_cross_multi(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p);
static int CPXPUBLIC callback_cross_single(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p);
    ...
    for (i = 0; i < inst->n_turbines; ++i)
    {
        for (j = i+1; j < inst->n_turbines; ++j)
        {
            for (k = 0; k < inst->n_turbines; ++k)
            {
                ...
                if (inst->model_type == LOOP_TASK_MODEL_TYPE && (inst->xstar[ypos(i, j, inst)] + inst->xstar[ypos(j, i, inst)] < EPSILON_SMALL)) continue;
                ...
                if (nzcnt > 0)
                {
                    ...
                    {
                        // LAZY CONSTRAINT
                        if (CPXaddlazyconstraints(env, lp, 1, nzcnt, &one, &less_equal, &izero, rmatind, rmatval, NULL)) print_error("Wrong CPXaddlazyconstraints() at no-cross constraints.");
                    }
                    ...
                    if (inst->model_type == NEW_ROWS_MODEL_TYPE)
                    {
                        ...
                        if (CPXnewrows(env, lp, 1, &one, &less_equal, NULL, NULL)) print_error("Wrong CPXnewrows() at no-cross constraints.");
                        for (h = 0; h < nzcnt; h++)
                        {
                          if (CPXchgcoef(env, lp, lastrow, rmatind[h], rmatval[h])) print_error("Wrong CPXchgcoef() at no-cross constraints.");
                        }
                    }
...
static int CPXPUBLIC callback_cross_multi(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p)
{
    ...
    *useraction_p = CPX_CALLBACK_DEFAULT;
    ...
    if (CPXgetcallbacknodex(env, cbdata, wherefrom, inst->xstar_vector[mythread], 0, inst->ncols - 1)) return 1; // y = current y from CPLEX-- y starts from position 0
    
    for (i = 0; i < inst->n_turbines; ++i)
    {
        for (j = i+1; j < inst->n_turbines; ++j)
        {
            if (inst->xstar_vector[mythread][ypos(i, j, inst)] + inst->xstar_vector[mythread][ypos(j, i, inst)] < EPSILON_SMALL) continue;
            ...
            for (k = 0; k < inst->n_turbines; ++k)
            {
                ...
                if (nzcnt > 0)
                {
                    ...
                    if (CPXcutcallbackadd(env, cbdata, wherefrom, nzcnt, 1.0, 'L', inst->indeces_vector[mythread], inst->values_vector[mythread], 0)) print_error("USER_separation: CPXcutcallbackadd error");
                    ncuts++;
                }
            }
        }
...
static int CPXPUBLIC callback_cross_single(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p)
{
    ...
    *useraction_p = CPX_CALLBACK_DEFAULT;
    ...
    for (i = 0; i < inst->n_turbines; ++i)
    {
        ...
        {
            if (xstar[ypos(i, j, inst)] + xstar[ypos(j, i, inst)] < EPSILON_SMALL) continue;
            ...
            for (k = 0; k < inst->n_turbines; ++k)
            {
                ...
                if (nzcnt > 0)
                {
                    ...
                    if (CPXcutcallbackadd(env, cbdata, wherefrom, nzcnt, 1.0, 'L', indeces, values, 0)) print_error("USER_separation: CPXcutcallbackadd error");
...
int hard_fix(instance *inst, CPXENVptr env, CPXLPptr lp)
{
    ...
    double *bd;
    int *indices;
    lu = calloc(inst->n_turbines * inst->n_turbines, sizeof(char));
    bd = calloc(inst->n_turbines * inst->n_turbines, sizeof(double));
    indices = calloc(inst->n_turbines * inst->n_turbines, sizeof(int));
